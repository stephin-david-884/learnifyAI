export const subMessages = {
    success: {
        PAYMENT_ORDER_CREATED: "Payment order created",
        SUBSCRIPTION_ACTIVATED: "Subscription activated",
        AVAILABLE_PLANS_FETCHED: "Fetched the available plans",
        ACTIVE_SUBSCRIPTION_FETCHED: "Fetched active subscription",
        USER_PAYMENTS_FETCHED: "User payments fetched",
        SUBSCRIPTION_PLAN_CREATED: "Subscription plan created",
        SUBSCRIPTION_PLAN_UPDATED: "Successfully updated subscription plan",
        SUBSCRIPTION_PLAN_DEACTIVATED: "Successfully deactivated subscription plan",
        CREDITS_FETCHED: "Credits fetched successfully",
        PAYMENTS_FETCHED: "Payments fetched successfully",
    },

    error: {
        INVALID_AMOUNT: "Invalid payment amount",
        FAILED_RAZORPAY_ORDER: "Failed to create Razorpay order",
        ACTIVE_SUBSCRIPTION_ALREADY_EXISTS: "An active subscription already exists",
        USER_NOT_FOUND: "User not found",
        SUBSCRIPTION_NOT_ACTIVE: "Subscription is not active",
        SUBSCRIPTION_PLAN_ALREADY_EXISTS: "Subscription Plan already exists",
        INVALID_CREDIT_AMOUNT: "Invalid credits",
        INSUFFICIENT_CREDITS: "Insufficient credits",
        NO_ACTIVE_SUBSCRIPTION: "There is no active subscription",
        PLAN_NOT_FOUND: "Subscription plan not found",
        PAYMENT_ALREADY_PROCESSED: "Payment already processed",
        PAYMENT_NOT_FOUND: "Payment not found",
        SUBSCRIPTION_NOT_FOUND: "Subscription not found",
        INVALID_PAYMENT_SIGNATURE: "Invalid payment signature",
        SUBSCRIPTION_PLAN_ALREADY_DEACTIVATED: "Subscription plan was already deactivated",
        CANNOT_DEACTIVATE_LAST_ACTIVE_PLAN: "Cannot deactivate last active plan",

    }
}